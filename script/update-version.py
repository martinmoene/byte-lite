...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+byte_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( byte_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+byte_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( byte_lite_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\byte_lite_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'byte_lite_version = "{major}.{minor}.{patch}"\n' )
    ...
    , ( 'include/nonstd/byte.hpp'
        , r'\#define\s+byte_lite_MAJOR\s+[0-9]+\s*$'
        , '#define byte_lite_MAJOR  {major}' )

    , ( 'include/nonstd/byte.hpp'
        , r'\#define\s+byte_lite_MINOR\s+[0-9]+\s*$'
        , '#define byte_lite_MINOR  {minor}' )

    , ( 'include/nonstd/byte.hpp'
        , r'\#define\s+byte_lite_PATCH\s+[0-9]+\s*$'
        , '#define byte_lite_PATCH  {patch}\n' )
)
